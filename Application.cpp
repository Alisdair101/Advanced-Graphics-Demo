...
void Application::InitGameObjectCubes(Geometry cubeGeometry, Material noSpecMaterial, Material shinyMaterial)
{
	...
	for (auto i = 0; i < 5; i++)
	{
		...
		case 1:
			gameObject->SetTextures(TextureManager::_pTextureList["Stone Texture"].get());
			break;

		case 2:
			gameObject->SetTextures(TextureManager::_pTextureList["Crate Texture"].get());
			break;
		...
		case 4:
			gameObject->SetTextures(TextureManager::_pTextureList["Cracked Rock Texture"].get());
			break;
		}
